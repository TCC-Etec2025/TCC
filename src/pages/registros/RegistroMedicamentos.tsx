...
type Medicamento = {
  ...
  data_fim?: string;
  recorrencia: string;
  horarios: string[]; // Array de horários no formato "HH:MM"
  dias_administracao: number[]; // Array de dias da semana (0-6)
  dias_horarios: string[];
  efeitos_colaterais?: string;
  observacoes?: string;
  saude_relacionada?: string;
  foto?: string; // URL ou base64 da imagem
  status: string;
  administracoes?: string;
  residente: {
    nome: string;
    quarto?: string;
  }
  ...
  */
  ...
  useEffect(() => {
    const fetchMedicamentos = async () => {
      try {// fazer ação e buscar info específica
        const { data, error } = await supabase // data se der certo, error se der errado
          .from("medicamento")
          .select(`
          id,
          created_at,
