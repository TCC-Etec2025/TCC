...
import { 
  ...
  Clock, Eye, Loader2, Calendar, UsersRound
} from "lucide-react";
      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row justify-between mb-6 sm:mb-8 gap-4">
          ...
          <div className="flex items-center space-x-2 text-sm text-gray-600">
            <Calendar size={20} className="text-odara-primary" />
        ...
        {visualizacao === "casa" && (
          <div className="mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
              ...
              {dados.length > 1 && (
                <button
                  onClick={() => selecionarFamiliar(0)}
                  className="flex items-center gap-2 px-4 py-2 bg-odara-primary text-white rounded-lg hover:bg-blue-700 transition font-medium text-sm"
                >
                  <Home size={18} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              {dados.map((d, idx) => (
                <div key={d.residente.id} onClick={() => selecionarFamiliar(idx)}
                  className="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow hover:shadow-md transition cursor-pointer border border-gray-100 group"
                >
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="relative w-12 h-12 sm:w-16 sm:h-16 rounded-full overflow-hidden bg-gray-100 shrink-0 border-2 border-white shadow">
                       {d.residente.foto ? <img src={d.residente.foto} alt={d.residente.nome} className="w-full h-full object-cover"/> : <User className="w-full h-full p-2 sm:p-4 text-gray-400"/>}
        ...
        {visualizacao === "familiar" && (
          <div className="mb-6 sm:mb-8">
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow border border-gray-100 p-4 sm:p-6">
               <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 mb-6">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-3 border-white shadow-lg overflow-hidden bg-gray-200">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 pt-4 sm:pt-6 border-t border-gray-100">
        ...
        {/* === DASHBOARD GRIDS - Estilo do Admin === */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="lg:col-span-3 space-y-4 sm:space-y-6">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div 
                   ...
                   className="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow hover:shadow-md transition cursor-pointer border border-gray-100"
                >
                   <div className="flex justify-between items-center mb-4">
                       <h3 className="font-bold text-odara-dark flex items-center gap-2 text-base sm:text-lg">
                         <div className="p-2 rounded-lg">
                           <Pill size={18} className="text-odara-primary"/>
                         </div>
                         Medicamentos
                       </h3>
                ...
                >
                   <div className="flex justify-between items-center mb-4">
                       <h3 className="font-bold text-odara-dark flex items-center gap-2 text-base sm:text-lg">
                         <div className="p-2 rounded-lg">
                           <Utensils size={18} className="text-odara-primary"/>
                         </div>
                         Cardápio (Hoje)
                       </h3>
                       <Eye size={18} className="text-gray-400"/>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                 ...
                 >
                    <div className="flex justify-between items-center mb-4">
                       <h3 className="font-bold text-odara-dark flex items-center gap-2 text-base sm:text-lg">
                         <div className="p-2 rounded-lg">
                           <Palette size={18} className="text-odara-primary"/>
                         </div>
                         Atividades (Hoje)
                       </h3>
                       <Eye size={18} className="text-gray-400"/>
          ...
          {/* COLUNA DIREITA (ALERTAS) - Estilo do Admin */}
          <div className="lg:col-span-1">
             <div className="bg-white rounded-xl sm:rounded-2xl shadow border border-gray-100 p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-4 sm:mb-6">
                   ...
                   <h2 className="font-bold text-lg text-odara-dark">Alertas</h2>
                   {totalAlertas > 0 && (
                ...
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                   ...
                   {proximoMedicamento && (
                      <div className="relative pl-4 border-l-2 border-blue-400 pb-4 bg-blue-50/50 p-3 rounded-r-lg">
                          ...
                          <p className="text-sm font-medium text-odara-dark truncate">{proximoMedicamento.nome_medicamento}</p>
                          <p className="text-xs text-blue-600 mt-1 font-semibold truncate">
                   ...
                   {ocorrenciasView.map(alert => (
                       <div key={alert.id} className="relative pl-4 border-l-2 border-red-200 pb-4">
                           <div className="absolute -left-[5px] top-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></div>
                           <h4 className="text-sm font-bold text-odara-dark truncate">{alert.titulo}</h4>
                           <p className="text-xs text-gray-500 mt-0.5 mb-2 truncate">
                             {alert.horario.slice(0,5)} • {alert.nome_residente}
                           </p>
                           <button 
                             onClick={() => setModalAberto('OCORRENCIA')}
                             className="text-xs font-medium text-odara-primary hover:underline"
                           >
                             Ver detalhes
                           </button>
                       </div>
